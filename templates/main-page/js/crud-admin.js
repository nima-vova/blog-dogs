...
(function($) {
	...
	var editElement = function (thas, id) {
        ...
        // on this (the button is pressed for editing and select the entire
		// line for editing in which the button was pressed)
		if (typeof id !== 'undefined'){
			var parentRow = $('tr.show-element.'+id+':first');
		}
		 else {
			//find a close parent on the attribute (in this case, the string we will edit)
            ...
            // split ("/") [2] - splits the string into an array by "/", we select the third element of the users-show array,
		...
		var fieldsWidth = [];
		var fieldsClassName = [];
		// run each cell of the string and write the value to the array.
        // children ('td'). length-1 reject the last cell because it contains the delete and editing buttons
		...
		//alert(fieldsWidth[0]);
		// delete all the descendants of the string (we make it empty to insert fields for editing)
		parentRow.empty();
		for (i = 0; i < fieldsValue.length; i++) {
			// make the first cell a non-variable variable (it is not possible to edit it) because it is id - it is not reregive, insert back as it was before
			if (i == 0) {
	...
	$.fn.crudAdmin = function(id) {
		// if the id parameter means it is not ajax so we call the function to edit the row in which there is an id
		if (id) {
        ...
        // (it is possible on the body, but it is more expedient to cling to the close parent),
		...
		this.on("click", ".delete-row", function () {
				// find the close parent of the attribute (in this case, the string we will edit)
				var parentRow = $(this).closest("tr");
				// we take the id of the element (to send to the server for deletion)
				fieldValue = parentRow.children('td').eq(0).text();
				// is defined from the address bar by the name of the entity over which the operations are performed
                // (pathname part url without host-/admin/users-show,
                // split ("/") [2] - splits the string into an array by "/", we select the third element of the array (for example, users),
        ...
        // after a positive server response, I change the string to the edited data
		this.on("click", ".save-row", function () {
			// we find the glittering ancestor of the attribute (in this case, the string we will store)
			var parentRow = $(this).closest("tr");
			// fieldsValue is the object, because it's better to pass ajax
			var fieldsValue = {};
			// will be the property of the fieldValue object with the name of the field and the value of this field
			var fieldsClassName = [];
			// is defined from the address bar by the name of the entity over which the operations are performed
            // (pathname part url without host- / admin / users-show,
